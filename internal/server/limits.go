...
func ProLimits() Limits {
	return Limits{
		MaxProjects: 0, MaxIssuesPerProject: 0,
		Milestones: true, Labels: true, Export: true,
		Tier: "pro",
	}
}
...
func DefaultLimits() Limits {
	...
	if key == "" {
		log.Printf("[license] No license key — free tier (2 projects, 25 issues each)")
		log.Printf("[license] Get Pro at https://stockyard.dev/bounty/")
		return FreeLimits()
	}
	if validateLicenseKey(key, "bounty") {
