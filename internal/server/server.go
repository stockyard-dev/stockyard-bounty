...
func New(db *store.DB) *Server {
	...
	s.mux.HandleFunc("GET /api/issues", s.list)
	s.mux.HandleFunc("POST /api/issues", s.create)
	s.mux.HandleFunc("GET /api/issues/{id}", s.get)
	s.mux.HandleFunc("DELETE /api/issues/{id}", s.del)
...
func (s *Server) health(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"status": "ok", "service": "stockyard-bounty", "time": time.Now().UTC().Format(time.RFC3339)})
}

func (s *Server) stats(w http.ResponseWriter, r *http.Request) {
	wj(w, map[string]any{"issues": s.db.Count()})
}

func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	items := s.db.List()
	if items == nil { items = []store.Issue{} }
	wj(w, map[string]any{"issues": items, "count": len(items)})
}
